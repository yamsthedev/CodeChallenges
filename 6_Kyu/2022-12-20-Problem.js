// 6 Kyu Problem - December 20th 2022
// How Much?
// Math.max(), Math.min(), .push(), for loop, modulus, mathematics, .map(), template literals
...
// 9 cars at Price with 1 Leftover --> (i - 1) % 9 == 0
// AND
// 7 cars at Price with 2 Leftover --> (i - 2) % 7 == 0
// Math.max(), Math.min(), .push(), for loop, modulus, mathematics, .map(), template literals
function howMuch(m, n) {
  ...
  for(let i = minNum; i <= maxNum; i++){
    if((i-1) % 9 == 0 && (i-2) % 7 == 0){
      ans.push([i, (i-2)/7, (i-1)/9])
    }
  }
  return ans.map(a => [`M: ${a[0]}`, `B: ${a[1]}`, `C: ${a[2]}`])
}