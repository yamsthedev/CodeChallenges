// 7 Kyu Problem - January 4th 2023
// Training JS #37: Unlock new weapon---RegExp Object
// str.search(regex), new Regex(), literal notation, .replace(), .split(), .match(regex), for of loop, .push(), .map()
...
function countAnimals(animals,count){
  ...
  for(let animal of count){
    let x = animals.match(new RegExp(animal,"g")) || []
    ans.push(x.length)
  }
  return ans
}
// .map(), .match(), new RegExp()
function countAnimals(animals,count){
