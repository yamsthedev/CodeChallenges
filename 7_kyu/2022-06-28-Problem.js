...
function seven(m) {
    let steps = 0
    let convertedString = m.toString()
    
    while(convertedString.length > 2){
      let firstDigits = convertedString.slice(0, convertedString.length - 1)
      let lastDigit = convertedString.slice(-1)
      convertedString = String(firstDigits - (2 * lastDigit))
      steps++
     }
     
    return [Number(convertedString), steps]
   }
