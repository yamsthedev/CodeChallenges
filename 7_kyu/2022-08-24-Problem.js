// 7 Kyu Problem - August 24th 2022
// Looking for a benefactor
// throw, Math.ceil(), averaging
...
function newAvg(arr, newavg) {
    //arr + x / (arr.length + 1) = newavg
    // x = newavg * (arr.length + 1) - arr
  let lastDono = Math.ceil(newavg * (arr.length + 1) - (arr.reduce((acc,curr) => acc + curr, 0)))
  if(lastDono <= 0){
